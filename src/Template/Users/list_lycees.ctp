<div class="row">
    <div class="col-sm-3 col-md-2 sidebar">
        ...
        <ul class="nav nav-sidebar">
            ...
            <li class="active"><?= $this->Html->link('Liste des lycées', ['controller' => 'users', 'action' => 'listLycees']) ?></li>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        <?= $this->Flash->render() ?>
        <h1 class="page-header">Liste des lycées</h1>
        <div class="table-responsive">
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th scope="col"><?= $this->Paginator->sort('Nom du lycée') ?></th>
            ...
            <?php foreach ($users as $user): ?>
                    <tr>
                        <td><?= h($user->highschool_name) ?></td>
                        <td><?= h($user->address) ?></td>
                        <td><?= h($user->email) ?></td>
                        <td><?= h($user->phone_number) ?></td>
                        <td><?= h($user->birth_date) ?></td>
                        <td class="actions">
                    <?= $this->Html->link('Détails', ['action' => 'view_user_admin', $user->id]) ?>
                    <?= $this->Form->postLink('Supprimer', ['action' => 'delete', $user->id], ['confirm' => __('Êtes-vous certain de vouloir supprimer le compte de ' . $user->first_name . ' ' . $user->last_name . ' ?')]) ?>
                        </td>
                    </tr>
            <?php endforeach; ?>
                </tbody>
